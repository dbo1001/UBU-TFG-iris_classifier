...
class Preprocess:
    ...
    def ajustar_input(self,file_path):
    ...
    def segmentar(self,file_path):
        global im
        top = Toplevel()
        top.title("U-net Output")
        model = load_model('Iris_unet_d5.h5')
        # predecimos(segementamos) la muestra
        result = model.predict(self.ajustar_input(file_path))
        # reescalamos a tamaño original
        segmented = self.morph_operator(result)
        segmented_path = os.path.join(os.getcwd(),"output.png")
        Label(top, text=segmented_path).pack()
